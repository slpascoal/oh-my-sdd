# Practical Example of a `constitution.md`

Assuming the "current project" is a modern web application (connecting to the user authentication system example discussed earlier), here is a highly effective `constitution.md` template — the same structure `oh-my-sdd-constitution` follows when generating `.oh-my-sdd/constitution.md`:
...
By generating a document like this from your project's actual code and configuration, `oh-my-sdd-constitution` ensures every subsequent phase — spec, plan, tasks, implementation — stays technically and architecturally aligned with decisions your team already made, instead of reinventing them.
