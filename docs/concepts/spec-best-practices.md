# How to Create 100% Effective `.md` Specs
...
This is exactly the hierarchy `oh-my-sdd` automates: `oh-my-sdd-constitution` → `oh-my-sdd-specify` → `oh-my-sdd-plan` → `oh-my-sdd-tasks` → `oh-my-sdd-implement`.
...
`oh-my-sdd-specify` writes every functional requirement in this syntax — see the [practical example](spec-example.md).
...
This is why `oh-my-sdd-tasks` breaks `plan.md` into small, atomic, sequential tasks instead of one large implementation step.
